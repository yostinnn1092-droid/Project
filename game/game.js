...
function conjureWater(g) {
  ...
  sparks(waterConjurePos, 0x9fdcff, 7, 5);
}
...
const WATER = {
  ...
  hint:     "Tap FLOW to throw · the water finds them on its own",
  regen:    2.0,
  // Slow on purpose. The whole point of this kit is that you WATCH the shot
  // bend around and land, and at the blade's speed there is nothing to watch.
  speed:    24,
  dmg:      130,
  pierce:   0,
  blastR:   0,
  ...
  // Spaced out rather than packed. At half this interval and full opacity the
  // droplets stacked several deep around the head and bleached the whole shot
  // white — the spray has to read as separate water, not as a fog bank.
  trail:    { geo: () => waterSprayGeo, mats: waterTrailMats, every: 0.022,
              life: 0.52, spread: 0.12, rise: 0.02, drift: [-1.6, -0.5],
              size: [0.8, 1.6], grow: 0.65, spin: [-4, 4] },
};
...
const CHARS = {
  ...
  hydromancer: {
    name: "HYDROMANCER",
    desc: "Nothing to lift. Water out of thin air.",
    power: "water",
    props: 0,
    cast: WATER,
    perk(lv) { return castCap(lv) + " streams held"; },
  },
};
