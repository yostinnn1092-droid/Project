...
// chain before a fuse you have to time or a turret you have to place.
//
// The prices are set against the SHAPE of the game, not spread evenly. There
// are eleven authored waves and the Maw ends them; everything past that is
// endless. A first pass at this put six of the thirteen prices beyond wave 11
// and the last at 22 — twice the campaign — which would have gated half the
// roster behind content most players never reach, turning a chase into dead
...
const PROFILE = {
  ...
  charBest: {},
  // The first attempt at a given day's run, and only the first: a daily is a
  // score people compare, so a retry that overwrote it would make the number
  // meaningless. Replays are still allowed, they just do not rewrite this.
  daily: { day: 0, wave: 0, score: 0, no: 0 },
};
...
function loadProfile() {
  ...
  // Scalars come off storage as whatever the JSON held: a hand-edited save, a
  // half-written one, or a field an older build spelled differently. That used
  // to be cosmetic — a wrong number on the menu. bestWave now GATES THE
  // ROSTER, and a string there fails every `>=` comparison, which locks all
  // fifteen characters including the two that are meant to be free and leaves
  // the picker with nothing selectable at all. Coerce rather than trust.
  const num = (v, dflt, min) => {
    const n = typeof v === "number" ? v : parseFloat(v);
    return Number.isFinite(n) ? Math.max(min, Math.floor(n)) : dflt;
  };
  PROFILE.bestWave = num(PROFILE.bestWave, 1, 1);
  PROFILE.best     = num(PROFILE.best, 0, 0);
  PROFILE.runs     = num(PROFILE.runs, 0, 0);
  PROFILE.kills    = num(PROFILE.kills, 0, 0);
  if (typeof PROFILE.bestRank !== "string") PROFILE.bestRank = "D";
  if (!PROFILE.seen || typeof PROFILE.seen !== "object") PROFILE.seen = {};

  if (!PROFILE.daily || typeof PROFILE.daily !== "object") PROFILE.daily = {};
  for (const k of ["day", "wave", "score", "no"])
    PROFILE.daily[k] = num(PROFILE.daily[k], 0, 0);
...
function recordRun() {
  ...
  // The day's run, recorded once. A daily is a number people compare, so a
  // second attempt must not overwrite the first — but the SAME run continuing
  // into endless should still improve it, which is what dailyOwned tracks.
  if (S.seed !== null && S.seed !== undefined) {
    if (PROFILE.daily.day !== S.seed || S.dailyOwned) {
      PROFILE.daily = { day: S.seed, no: S.dailyNo, wave: S.wave, score: S.score };
      S.dailyOwned = true;
    }
  }
...
// A boss carries a bar, and NOTHING on the damage path repainted it: only
// kills and wave events called updateHUD. So the plates and the core froze at
// whatever they read when the last body died — and the moment that matters is
// precisely the one with no bodies left, alone with the boss at the end of the
// fight. Measured: breaking all four plates and taking the core to 45% left
// the bar reading ARMOURED at 100%/100%, and a manual updateHUD immediately
// corrected it to EXPOSED at 0%/45%.
//
// Wrapped rather than patched at the exits. There are THREE — two inside the
// plate branch, which absorbs the hit and returns early, and the ordinary one
// at the bottom — and a first attempt that only covered the bottom missed
// every plate hit, which is most of the fight. Guarded on `boss` because
// updateHUD walks the walker list and a zombie taking a chip does not need
// that; a boss does, and there is at most one.
function damageWalker(w, amount, dir, knock, kind) {
  const isBoss = !!(w && w.boss && !w.dead);
  damageWalkerFrom(w, amount, dir, knock, kind);
  if (isBoss) updateHUD();
}

function damageWalkerFrom(w, amount, dir, knock, kind) {
  if (w.dead) return;
...
const S = {
  ...
  // null for a normal run; the day's number for a daily. Only buildWave reads
  // it, so nothing else in the game has to know a run is seeded.
  seed:null, dailyNo:0, dailyOwned:false,
...
// ─────────────────────────────────────────────────────────── seeded runs
// A daily has to deal every player the same waves, and seeding once at the
// start does NOT achieve that: the moment two players fight differently they
// draw different amounts of randomness, and by wave 9 their modifiers have
// come apart. Particles alone would do it.
//
// So the seed is applied per WAVE rather than per run, derived from (seed,
// wave number), around the one function that builds content. Everything
// outside that window — crits, sparks, whatever the fight happens to look
// like — draws from the normal stream and cannot push the content out of
// step. Wave 9 is wave 9 for everyone, however they got there.
//
// Math.random is swapped rather than every call site rewritten: there are 48
// of them, most in visual code where determinism is neither needed nor
// wanted, and the swap is restored in a finally so a throw inside cannot
// leave the game deterministic for the rest of the session.
...
function waveSeed(seed, n) {
  return (Math.imul(seed ^ 0x9E3779B9, 2654435761) ^ Math.imul(n + 1, 40503)) | 0;
}
...
// The day itself is the seed, in UTC so the world plays the same run rather
// than a new one rolling over timezone by timezone.
function todaySeed(d) {
  d = d || new Date();
  return d.getUTCFullYear() * 10000 + (d.getUTCMonth() + 1) * 100 + d.getUTCDate();
}
// A short human number for the share line. Counted from the day this shipped.
function dailyNumber(d) {
  d = d || new Date();
  const epoch = Date.UTC(2026, 8, 1);
  return Math.max(1, Math.floor((Date.UTC(d.getUTCFullYear(), d.getUTCMonth(),
    d.getUTCDate()) - epoch) / 86400000) + 1);
}

function buildWave(n) {
  // A normal run stays as random as it ever was.
  if (S.seed === null || S.seed === undefined) return buildWaveFrom(n);
  return withSeed(waveSeed(S.seed, n), () => buildWaveFrom(n));
}

function buildWaveFrom(n) {
  clearAll();
  rollModifier(n);
...
function shareLine() {
  // A daily is comparable, so it leads with the thing another player can
  // answer. A plain score is a number about you; "wave 12 on daily #4" is an
  // invitation.
  if (S.seed !== null && S.seed !== undefined) {
    return "Kinesis daily #" + S.dailyNo + " — wave " + S.wave +
           " · " + S.score.toLocaleString() + " · " + CHAR.name;
  }
...
// Everything that has to come down when a card goes up. The draft and the end
// screens both did this inline, four identical lines each, and both forgot the
// same fifth: a toast raised during the wave keeps its full opacity over the
// card. On a phone in landscape it lands at 343px of a 390px screen — directly
// under the buttons — so a run ended right after a wave condition was
// announced put "EVERYTHING IS PLATED. BLASTS IGNORE PLATE." across the bottom
// of the death screen. Written once here so the next card cannot forget it.
function leavePlayView() {
  clearTimeout(toastT);
  el("toast").classList.remove("show");
  // The hold window has to go with it. It suppresses lower-priority toasts
  // until it expires, so a run ended moments after an unlock announcement
  // would carry that silence into the NEXT run and swallow its opening hint —
  // trading a visible bug for an invisible one.
  toastHold = 0;
...
function restart() {
  ...
  // Try again leaves the daily behind: it is one attempt on a fixed set of
  // waves, so a retry is an ordinary run rather than a second go at today's.
  S.seed = null; S.dailyNo = 0; S.dailyOwned = false;
...
// Today's run. Same waves, same conditions, same order, for everyone who
// plays it — which is what makes the score worth telling someone.
function startDaily() {
  audioInit();
  restart();                 // clears any previous run, and its seed
  S.seed = todaySeed();
  S.dailyNo = dailyNumber();
  S.dailyOwned = false;
  buildWave(S.wave);         // rebuild wave 1 now that the seed is set
  toast("Daily #" + S.dailyNo + " — same waves for everyone", 3400, true);
}
...
// Today's line under the daily button: an invitation before it is played, a
// score afterwards. Painted alongside the rest of the menu so a restored menu
// gets it too.
function paintDaily() {
  const line = el("dailyLine"), btn = el("dailyBtn");
  const no = dailyNumber(), today = todaySeed();
  if (btn) btn.textContent = "Daily run #" + no;
  if (!line) return;
  const d = PROFILE.daily;
  line.innerHTML = (d && d.day === today && d.wave)
    ? "Today: <b>wave " + d.wave + "</b> · " + (d.score || 0).toLocaleString() +
      " — same waves for everyone"
    : "One fixed run a day · everybody gets the same waves";
...
function toMenu() {
  ...
  const d = el("dailyBtn");
  if (d) d.addEventListener("click", startDaily);
  el("modName2").classList.remove("show");
  el("bossBar").classList.remove("show");
  el("overlay").classList.remove("hide");
  ["hud","touch","cross"].forEach(i => el(i).classList.add("hide"));
