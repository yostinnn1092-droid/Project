...
const MODIFIERS = [
  ...
  { id:"scarcity", name:"SCARCITY",    desc:"Little to throw. Focus returns twice as fast.",
...
function rollModifier(n) {
  ...
  const pool = MODIFIERS.filter(m => !(m.id === "glass" && hero.hp <= 2));
...
function buildWave(n) {
  ...
  for (const key in OBJECTS) {
    ...
    const pm = (WMOD.props && WMOD.props[key] !== undefined ? WMOD.props[key] : 1)
             * WMOD.propsAll;
    const n2 = Math.max(1, Math.round(def.count * (1.4 + n*0.15) * pm));
    for (let i = 0; i < n2; i++) {
      const a = rand(0,Math.PI*2);
  ...
  if (WMOD.name) setTimeout(() => { banner(WMOD.name); toast(WMOD.desc, 3200); }, 420);
  updateHUD();
}
...
function grabReach() {
  return CFG.grabRadius + MOD.grabR + (OD.on ? CFG.odGrabR : 0);
}
...
function step(dt) {
  ...
  S.focus = OD.on ? 1 : Math.min(1, S.focus + CFG.focusRegen*MOD.focusRegen*WMOD.focus*dt);
  ...
  if (S.recycleT <= 0) {
    ...
    let inReach = 0;
    for (const o of rocks)
      if (!o.gone && !o.held && o.pos.distanceTo(hero.pos) < grabReach()) inReach++;
    if (inReach < 6) {
      let moved = 0;
      for (const o of rocks) {
        if (moved >= 3) break;
