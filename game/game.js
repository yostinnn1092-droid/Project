...
function stepRing(dt) {
  ...
  if (ringState.disc)
    ringState.disc.material.opacity = (0.24 + Math.sin(S.t*4.2)*0.06) * L.glow;
  if (ringState.light)
    ringState.light.intensity = (1.4 + Math.sin(S.t*6.5)*0.35) * L.glow;
...
// telekinesis reads on the OBJECTS and not only on the character. Pooled
// rather than parented per prop: there are up to ~170 props in an arena and
// at most a dozen under control at once, so a couple of dozen meshes is the
// honest number.
//
// Restored by request. Note this is the ORIGINAL rim shell, not the tall
// procedural plume that briefly replaced it — the plume was the thing removed
// for being too loud, and the tethers that took over as the control signal
// have since gone too, which left controlled props with no marker at all.
//
// Additive blending, so it adds light rather than tinting — and it feeds the
// bloom pass on the high quality tier, which is what turns it from a coloured
// shell into a glow.
...
function updateAuras() {
  ...
  const claim = (o, col, base, pulseHz) => {
    ...
    // Just over the prop's own size, and following the prop's own scale so the
    // zoom that shrinks a held object shrinks its aura too. The shell is
    // BackSide, so the prop — which writes depth — hides everything except the
    // sliver that peeks out around its silhouette. That sliver is the glow. At
    // 1.85 the whole far hemisphere showed and each one read as a filled
    // bubble instead, with seven merging into a single pink mass.
