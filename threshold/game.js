...
function generateFloor(n) {
  ...
  if (isBoss) {
    ...
    while (spent < budget && guard++ < 60) {
      ...
      let kind = "husk";
      const roll = rnd();
      if (n >= CFG.eliteFrom && roll < 0.16) kind = "warden";
      else if (roll < 0.42) kind = "mote";
      else if (roll < 0.66) kind = "lash";
      spawnEnemy(kind, ex, ez, n);
      spent += kind === "warden" ? 3 : 1;
    }
  }
...
function updatePlayer(dt) {
  ...
  if (P.dodgeT > 0) {
    ...
    if (mv.lengthSq() > 0.01) {
      mv.normalize();
      const spd = CFG.moveSpeed * (P.atkT>0?0.42:1);
    ...
    if (e.stun <= 0 && e.aggro) {
      if (e.boss) updateBoss(e, dt, toP, dist);
        ...
        if (e.t <= 0) {
          ...
          if (e.def.ranged) {
            spawnBolt(e.pos, toP, e.dmg, true, 15);
