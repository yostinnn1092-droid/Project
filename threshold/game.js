...
function generateFloor(n) {
  ...
  if (isBoss) {
    ...
    while (spent < budget && guard++ < 60) {
      ...
      let kind = "husk";
      const roll = rnd();
      if (n >= CFG.eliteFrom && roll < 0.13) kind = "warden";
      else if (n >= 2 && roll < 0.25) kind = "bell";
      else if (n >= 2 && roll < 0.38) kind = "scribe";
      else if (roll < 0.56) kind = "mote";
      else if (roll < 0.76) kind = "lash";
      spawnEnemy(kind, ex, ez, n);
      spent += kind === "warden" ? 3 : (kind === "bell" ? 2 : 1);
    }
  }
...
function spawnBolt(from, dir, dmg, hostile, speed) {
  ...
  G.projectiles.push({ mesh:m, pos:m.position, dir:dir.clone().normalize(),
                       spd:speed||CFG.boltSpeed, dmg, hostile, life:3.2 });
}

function spawnHazard(x,z,r,life,dps,friendly) {
  ...
  G.root.add(m);
  G.hazards.push({ mesh:m, x, z, r, life, dps, friendly, tick:0 });
}
...
function updateEnemies(dt) {
  for (const e of G.enemies) {
    ...
    if (!e.aggro && dist < 17) e.aggro = true;
        ...
        if (e.t <= 0) {
          ...
          if (e.def.ranged) {
            ...
            for (let k = 0; k < n; k++) {
              ...
              spawnBolt(e.pos, dir, e.dmg, true, 15);
            }
          } else if (e.def.aoe) {
